...
uses
  ...
  TabFunctions, Spin, GNSSObjects, GeoString, ImgList, Menus;
...
inherited;
  ...
  begin
    ...
    //FillRect(Rect);
    IcoList.Draw((Control as TListBox).Canvas, Rect.Left, Rect.Top-2, IcoN);
    TextOut(Rect.Left + 20, Rect.Top, (Control as TListBox).Items[Index]);
    if odFocused In State then begin
      Brush.Color := (Control as TListBox).Color;
      DrawFocusRect(Rect);
...
begin
  ...
  begin
     ...
     begin
        ...
        A[j] := 2;
        B[j] := GNSSSessions[GetGNSSSessionNumber(GNSSVectors[I].RoverId)];
        C[j] := GNSSSessions[GetGNSSSessionNumber(GNSSVectors[I].BaseId)];
     end;
  end;
