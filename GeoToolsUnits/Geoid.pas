...
begin
   ...
   repeat
     ...
     if Pos(NameIDS, AnsiLowerCase(Str)) > 0 then
        GeoidList[N].NameID := Trim(GetCols(Str,1,GetColCount(Str,' '),' ',false));

     if Pos(CaptionS, AnsiLowerCase(Str)) > 0 then
        GeoidList[N].Caption := Trim(GetCols(Str,1,GetColCount(Str,' '),' ',false));
...
//        Str := S2[K];
        ...
        if Pos(NameIDS, AnsiLowerCase(Str)) > 0 then
            GeoidsMetaData[j].NameID := Trim(GetCols(Str,1,GetColCount(Str,' '),' ',false));

        if Pos(CaptionS, AnsiLowerCase(Str)) > 0 then
            GeoidsMetaData[j].Caption := Trim(GetCols(Str,1,GetColCount(Str,' '),' ',false));
