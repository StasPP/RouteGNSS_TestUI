...
uses FLoader, UStartProcessing, UGNSSPointSettings, UAntProp;
...
begin
    ...
    if GNSSSessions[ActiveGNSSSessions[I]].Antenna.AntName <>
     ...
     // 1) Analyse is N - E equal

     // 2) Recompute AntH ftom AntH_u, check if it is the same

     // 2.5) Ask user!

     // 3) if not the same - go to solutions - recompute XYZ + d(XYZ) by ENU
